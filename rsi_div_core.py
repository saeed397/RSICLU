...
نکات کلیدی وفاداری به سورس اصلی:
  ...
  - نقطه‌ی "پایان" فلش دقیقاً همان bar_index کندلی است که در آن RSI از
    منطقه‌ی oversold/overbought خارج می‌شود (کندل تأیید)، نه لزوماً کندل
    دقیق کف/سقف RSI. نقطه‌ی "شروع" فلش، کندل کف/سقف تأیید شده‌ی *قبلی*
    (lastlowestbi[1] / lasthighestbi[1]) است. این دقیقاً همان چیزی است که
    سورس Pine با line.new(x1=bar_index, x2=lastlowestbi[1], ...) انجام می‌دهد.
"""
